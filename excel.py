...
def output_excel(json_data):
    ...
    workbook_data = {}
    list_dfs = []   # Added from old code
    name_dic = {}   # Added from old code
    ...
    try:
        ...
        with pd.ExcelWriter(output_filename, engine="openpyxl") as writer:
            for idx, (sheet_name, data) in enumerate(workbook_data.items()):
                if data:
                    ...
                    # Add DataFrame to list_dfs (Old functionality)
                    list_dfs.append(df_uniq)
                    
                    # Add sheet name to name_dic (Old functionality)
                    name_dic[idx] = sheet_name if len(sheet_name) <= 31 else sheet_name.replace("Statement of", "")
                    
                    # Write cleaned dataframe to Excel
                    df_uniq.to_excel(writer, index=False, sheet_name=name_dic[idx])
                    ...
                    if not df_duplicates.empty:
                        ...
                        if len(df_unique) == 0:
                            ...
                            df_no_redundancy.to_excel(writer, index=False, sheet_name=f"{sheet_name[:25]}")
